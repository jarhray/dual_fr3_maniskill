...
class UsbCableBridge(ManiSkillBridge):
    def __init__(self):
        ...
        if self.sim.cable is not None:
            self.get_logger().info("USB fixed to left gripper; drag left_fr3_arm in RViz, then Plan & Execute. "
                                   "Orange cable is measured MPM state. Simulation may run slower than wall time.")
    ...
    def create_simulation(self, assets):
        ...
        self.cable_config = load_config(config_path)
        return UsbCableSimulation(assets, cable_config=self.cable_config,
        ...
        try:
            ...
            for side in list(self.arms):
                self.finish_arm(side, FollowJointTrajectory.Result.PATH_TOLERANCE_VIOLATED,
                                "MPM stopped: " + self.failure)
            for side in list(self.grippers):
                self.finish_gripper(side)
            self.get_logger().error("MPM paused: " + self.failure + "; call /usb_cable_demo/reset to restart")
            self.diag_pub.publish(DiagnosticArray(status=[DiagnosticStatus(
                name="usb_cable_demo/MPM", level=DiagnosticStatus.ERROR, message=self.failure)]))
    ...
    def publish_state(self, q, v):
        ...
        stride = self.cable_config["mpm"]["marker_stride"]
        indices = list(range(0, len(center) - 1, stride)) + [len(center) - 1]
        line = Marker()
        line.header.frame_id, line.header.stamp = "world", stamp(self.sim.time)
        line.ns, line.id, line.type, line.action = "mpm_cable", 0, Marker.LINE_STRIP, Marker.ADD
        ...
        if wall - self.last_diagnostic >= 1.:
            ...
            status = DiagnosticStatus(name="usb_cable_demo/MPM", hardware_id="ManiSkill2 CUDA",
                level=DiagnosticStatus.OK, message="MPM with axial XPBD fibers; material is not calibrated",
